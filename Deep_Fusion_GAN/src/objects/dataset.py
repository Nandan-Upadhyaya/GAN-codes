...
class DFGANDataset(Dataset):
    def __init__(self, data_dir: str, split: str = "train", transform: Optional[Compose] = None):
        ...
        self.images_dir = os.path.join(self.data_dir, "birds")  # Changed from CUB_200_2011/images

        # FIX: Look for bounding_boxes.txt in the correct location
        self.bbox_path = os.path.join(self.data_dir, 'bounding_boxes.txt')
        if not os.path.isfile(self.bbox_path):
            # The file is actually in D:\GAN-codes\Deep_Fusion_GAN\data\
            self.bbox_path = os.path.join('..', 'data', 'bounding_boxes.txt')
            if not os.path.isfile(self.bbox_path):
                self.bbox_path = os.path.join(os.getcwd(), 'data', 'bounding_boxes.txt')

        self.images_path = os.path.join(self.data_dir, "images.txt")  # Changed from CUB_200_2011/images.txt
        if not os.path.isfile(self.images_path):
            self.images_path = os.path.join('..', 'data', 'images.txt')
            if not os.path.isfile(self.images_path):
                self.images_path = os.path.join(os.getcwd(), 'data', 'images.txt')
    ...
    def _load_bbox(self) -> Dict[str, List[int]]:
        # Use sep='\s+' instead of delim_whitespace for future compatibility
        df_bbox = pd.read_csv(self.bbox_path, sep='\s+', header=None).astype(int)

        df_image_names = pd.read_csv(self.images_path, sep='\s+', header=None)  # Fixed delim_whitespace warning
